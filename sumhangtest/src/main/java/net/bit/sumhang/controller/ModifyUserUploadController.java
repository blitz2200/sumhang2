...
public class ModifyUserUploadController {

	@Autowired
	private SqlSession sqlSession;
	
	@RequestMapping(value = "/modifyUserPhoto", method = RequestMethod.POST)
		public @ResponseBody String editTripFile(HttpServletRequest req, @RequestPart MultipartFile modifyUserPhoto){
		System.out.println("넘어온 파일 데이타는?"+modifyUserPhoto);
			
		//파일 유효성 체크	
		if(!modifyUserPhoto.isEmpty()){
				try{				
					//바이트에 넘어온 파일 저장
					byte[] bytes=modifyUserPhoto.getBytes();
				
					String editTripPhotoFile=modifyUserPhoto.getOriginalFilename()+".jpg";
					System.out.println("수정할 업로드 파일 이름:"+ editTripPhotoFile);
