...
@Controller
public class EditTripDetailUploadController {
	
	@Autowired
	private SqlSession sqlSession;
	
	@RequestMapping(value = "/editTripGalleryPhoto", method = RequestMethod.POST)
		public @ResponseBody String editTripFile(HttpServletRequest req, @RequestPart MultipartFile editTripGalleryPhoto){
		System.out.println("넘어온 파일 데이타는?"+editTripGalleryPhoto);
			
		//파일 유효성 체크	
		if(!editTripGalleryPhoto.isEmpty()){
				try{				
					//바이트에 넘어온 파일 저장
					byte[] bytes=editTripGalleryPhoto.getBytes();
				
					String editTripPhotoFile=editTripGalleryPhoto.getOriginalFilename()+".jpg";
					System.out.println("업로드 파일 이름:"+ editTripPhotoFile);
					...
					//서버에 파일 저장
					File serverFile = new File(dir.getAbsolutePath() + File.separator + editTripPhotoFile);
					BufferedOutputStream stream = new BufferedOutputStream(new FileOutputStream(serverFile));
					stream.write(bytes);
					stream.close();
					
					System.out.println("여행게시판사진성공="+editTripPhotoFile);
					//리사이즈할 원본 파일 폴더경로
					
					//리사이즈 할 파일 파일확장자까지;
					String sumnailePath=req.getSession().getServletContext().getRealPath("resources/images/tripPhotoFile");
					
					File sumnailFile =new File(sumnailePath+ File.separator + editTripPhotoFile);
					...
					//리사이즈할 파일 폴더 경로
					String sumNailImage = "s_"+editTripPhotoFile;
